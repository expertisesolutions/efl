...
test_normal_string(void)
{
   ...
   ret |= evil_fnmatch(pattern, string, flags);
...
test_unix_path(void)
{
   ...
   ret |= evil_fnmatch(pattern, string_valid, flags);
   printf("test unix path (T): %cok!\n", !ret?:'n');

   ret |= !evil_fnmatch(pattern, string_invalid, flags);
...
test_flag_pathname(void)
{
   ...
   ret |= evil_fnmatch(pattern_valid, string, 0);
   printf("test FNM_PATHNAME flag (T): %cok!\n", !ret?:'n');
   
   ret |= evil_fnmatch(pattern_invalid, string, 0);
   printf("test FNM_PATHNAME flag (T): %cok!\n", !ret?:'n');

   ret |= evil_fnmatch(pattern_valid, string, flags);
   printf("test FNM_PATHNAME flag (T): %cok!\n", !ret?:'n');
   
   ret |= !evil_fnmatch(pattern_invalid, string, flags);
...
test_flag_noescape(void)
{
   ...
   ret |= !evil_fnmatch(pattern, string_valid, 0);
