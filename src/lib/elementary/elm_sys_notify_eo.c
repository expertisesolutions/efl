...
ELM_API_WEAK EFL_FUNC_BODYV(elm_obj_sys_notify_servers_set, Eina_Bool, 0, EFL_FUNC_CALL(servers), Elm_Sys_Notify_Server servers);

Elm_Sys_Notify_Server _elm_sys_notify_servers_get(const Eo *obj, Elm_Sys_Notify_Data *pd);

ELM_API_WEAK EFL_FUNC_BODY_CONST(elm_obj_sys_notify_servers_get, Elm_Sys_Notify_Server, 0);

Elm_Sys_Notify *_elm_sys_notify_singleton_get(void);

ELM_API_WEAK Elm_Sys_Notify * elm_obj_sys_notify_singleton_get(void)
{
   elm_sys_notify_class_get();
   return _elm_sys_notify_singleton_get();
}
ELM_API_WEAK Elm_Sys_Notify * elm_sys_notify_singleton_get(void)
{
   elm_sys_notify_class_get();
   return _elm_sys_notify_singleton_get();
}
