#ifndef EFL_EINA_BOOST_TT_IS_ABSTRACT_CLASS_HPP
#define EFL_EINA_BOOST_TT_IS_ABSTRACT_CLASS_HPP
...
//              and John Maddock.
//  - Dec 2004: Added new config macro EFL_EINA_BOOST_NO_IS_ABSTRACT which causes the template
//              to degrade gracefully, rather than trash the compiler (John Maddock).
//

#include <eina_boost/type_traits/intrinsics.hpp>
#ifndef EFL_EINA_BOOST_IS_ABSTRACT
...
#ifdef EFL_EINA_BOOST_NO_IS_ABSTRACT
#include <eina_boost/type_traits/is_polymorphic.hpp>
#endif
#endif
...
#ifdef EFL_EINA_BOOST_IS_ABSTRACT
template <class T>
struct is_abstract_imp
{
   EFL_EINA_BOOST_STATIC_CONSTANT(bool, value = EFL_EINA_BOOST_IS_ABSTRACT(T));
};
#elif !defined(EFL_EINA_BOOST_NO_IS_ABSTRACT)
template<class T>
struct is_abstract_imp2
{
   ...
   //
   EFL_EINA_BOOST_STATIC_ASSERT(sizeof(T) != 0);

   // GCC2 won't even parse this template if we embed the computation
   // of s1 in the computation of value.
#ifdef __GNUC__
   EFL_EINA_BOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(is_abstract_imp2<T>::template check_sig<T>(0)));
#else
#if EFL_EINA_BOOST_WORKAROUND(EFL_EINA_BOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(push)
#pragma warning(disable:6334)
#endif
   EFL_EINA_BOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(check_sig<T>(0)));
#if EFL_EINA_BOOST_WORKAROUND(EFL_EINA_BOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(pop)
#endif
#endif
...
struct is_abstract_imp
{
   typedef is_abstract_select< ::efl_eina_boost::is_class<T>::value> selector;
   typedef typename selector::template rebind<T> binder;
   typedef typename binder::type type;

   EFL_EINA_BOOST_STATIC_CONSTANT(bool, value = type::value);
};
...
EFL_EINA_BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::efl_eina_boost::detail::is_abstract_imp<T>::value)
#else
EFL_EINA_BOOST_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::efl_eina_boost::detail::is_polymorphic_imp<T>::value)
#endif
...
#endif //EFL_EINA_BOOST_TT_IS_ABSTRACT_CLASS_HPP
