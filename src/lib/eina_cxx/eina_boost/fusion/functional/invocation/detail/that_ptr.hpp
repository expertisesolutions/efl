...
#if !defined(EFL_EINA_BOOST_FUSION_FUNCTIONAL_INVOCATION_DETAIL_THAT_PTR_HPP_INCLUDED)
#define EFL_EINA_BOOST_FUSION_FUNCTIONAL_INVOCATION_DETAIL_THAT_PTR_HPP_INCLUDED

#include <eina_boost/get_pointer.hpp>
...
namespace efl_eina_boost { namespace fusion { namespace detail
{
    ...
    struct that_ptr
    {
      ...
      public:
        ...
        static inline pointee * get(pointee & x)
        {
            return efl_eina_boost::addressof(x); 
        }

        template <typename T> static inline pointee * get(T & x)
        {
            return do_get_pointer(x, efl_eina_boost::addressof(x)); 
        }
    };
    ...
    {
        using efl_eina_boost::get_pointer;
        void const * EFL_EINA_BOOST_TT_DECL get_pointer(...); // fallback
