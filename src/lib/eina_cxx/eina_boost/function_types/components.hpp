...
#ifndef EFL_EINA_BOOST_FT_COMPONENTS_HPP_INCLUDED
#define EFL_EINA_BOOST_FT_COMPONENTS_HPP_INCLUDED
...
#include <eina_boost/mpl/vector/vector0.hpp>

#if EFL_EINA_BOOST_WORKAROUND(__BORLANDC__, <= 0x565)
#   include <eina_boost/type_traits/remove_cv.hpp>

#   include <eina_boost/mpl/identity.hpp>
#   include <eina_boost/mpl/bitand.hpp>
...
#   include <eina_boost/mpl/joint_view.hpp>
#   include <eina_boost/mpl/equal_to.hpp>
#   include <eina_boost/mpl/copy.hpp>
#   include <eina_boost/mpl/front_inserter.hpp>

#   include <eina_boost/function_types/detail/classifier.hpp>
#endif

#ifndef EFL_EINA_BOOST_FT_NO_CV_FUNC_SUPPORT
#   include <eina_boost/mpl/remove.hpp>
#endif
...
#     include <eina_boost/mpl/vector/vector30.hpp>
#   elif EFL_EINA_BOOST_FT_MAX_ARITY < 40
#     include <eina_boost/mpl/vector/vector40.hpp>
#   elif EFL_EINA_BOOST_FT_MAX_ARITY < 50
#     include <eina_boost/mpl/vector/vector50.hpp>
#   endif
#else
#   include <eina_boost/function_types/detail/classifier.hpp>
#endif

#include <eina_boost/function_types/detail/class_transform.hpp>
...
{ 
  namespace function_types 
  {
    ...
    { 
      ...
      EFL_EINA_BOOST_MPL_AUX_LAMBDA_SUPPORT(2,components,(T,ClassTypeTransform))
    };
...
#if !EFL_EINA_BOOST_FT_NO_CV_FUNC_SUPPORT
...
#if !EFL_EINA_BOOST_WORKAROUND(__BORLANDC__, <= 0x565)
#   define EFL_EINA_BOOST_FT_variations EFL_EINA_BOOST_FT_pointer|EFL_EINA_BOOST_FT_member_pointer
...
#else  
#   define EFL_EINA_BOOST_FT_variations EFL_EINA_BOOST_FT_pointer
    ...
    struct components_impl<T C::*, L>
      : detail::retagged_if
        < detail::components_impl<typename efl_eina_boost::remove_cv<T>::type *, L>
    ...
    { typedef char (& type)[1]; EFL_EINA_BOOST_STATIC_CONSTANT(std::size_t, value = 1); };
    template<typename T> struct encode_cv<T const *>
    { typedef char (& type)[2]; EFL_EINA_BOOST_STATIC_CONSTANT(std::size_t, value = 2); };
    template<typename T> struct encode_cv<T volatile *>
    { typedef char (& type)[3]; EFL_EINA_BOOST_STATIC_CONSTANT(std::size_t, value = 3); };
    template<typename T> struct encode_cv<T const volatile *> 
    { typedef char (& type)[4]; EFL_EINA_BOOST_STATIC_CONSTANT(std::size_t, value = 4); };
    ...
    { 
      EFL_EINA_BOOST_STATIC_CONSTANT(std::size_t, value = 
    ...
    template<typename T> struct cv_tag_mfp
      : detail::cv_tag_mfp_impl
        < ::efl_eina_boost::function_types::detail::encode_mfp_cv<T>::value >
    { };
    ...
    template<typename T, typename D, typename L> struct bcc_class_transform 
      : bcc_class_transform_impl
        < typename decode_cv
          < T
          , ::efl_eina_boost::function_types::detail::encode_mfp_cv<D>::value 
          >::type
        , L
        > 
    { };
...
#define EFL_EINA_BOOST_FT_al_path eina_boost/function_types/detail/components_impl
#include <eina_boost/function_types/detail/pp_loop.hpp>

  } } // namespace function_types::detail

  EFL_EINA_BOOST_TT_AUX_TEMPLATE_ARITY_SPEC(2,function_types::components)

} // namespace ::boost

#include <eina_boost/function_types/detail/components_as_mpl_sequence.hpp>
