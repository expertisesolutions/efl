...
#ifndef EFL_EINA_BOOST_FT_DETAIL_CV_TRAITS_HPP_INCLUDED
#define EFL_EINA_BOOST_FT_DETAIL_CV_TRAITS_HPP_INCLUDED
...
struct cv_code
{
  ...
  EFL_EINA_BOOST_STATIC_CONSTANT(std::size_t, value = 
      sizeof(::efl_eina_boost::function_types::detail::switch_cv(
         ::efl_eina_boost::function_types::detail::ref_to_ptr(_t) ) ));
};

template<typename T> struct cv_traits 
{
  typedef typename efl_eina_boost::function_types::detail::cv_tag_impl< 
    ::efl_eina_boost::function_types::detail::cv_code<T>::value >::type
  tag;

  // may require Boost.TypeTraits broken compiler specializations
  // to work
  typedef typename efl_eina_boost::remove_cv<
              typename efl_eina_boost::remove_pointer<
                  typename efl_eina_boost::remove_reference<T>::type 
              >::type 
          >::type type; 
};
#endif
