      ...
      case EMODEL_EIO_FILE_TYPE_DIR:
         {
            ...
            strncpy((char*)child->fullpath, child->priv->path, strlen(child->priv->path));
            strncat((char*)child->fullpath, "/", 1);
            strncat((char*)child->fullpath, child->name, strlen(child->name));
